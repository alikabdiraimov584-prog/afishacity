...
// ---- Разбор фразы на остановки ----
// Словарь занятий решает не только «что искать», но и «считать ли это планом
...
test("обычные разговорные формулировки собираются в план", ()=>{
  const cases=[
    ...
    ["бар потом клуб потом кальян",["бар","ночной клуб","кальянная"]],
    ["спеть в караоке а потом выпить",["караоке","бар"]],
    ["поужинать, потом в бар",["ужин ресторан","бар"]],
    ["перекусить затем кофе",["ужин ресторан","кофейня"]]
  ];
...
test("конец вечера не считается точкой и не рушит план", ()=>{
  ...
  assert.deepEqual(parseStops("поесть потом домой"),[],"одна точка — это не маршрут");
});

test("непонятное занятие по-прежнему планом не считается", ()=>{
  // Иначе «поесть, а потом к маме» превратится в поиск мамы по городу.
  assert.deepEqual(parseStops("поесть, а потом к маме"),[]);
  assert.deepEqual(parseStops("просто бар"),[]);
});
...
test("зрелища не сваливаются в одну кучу", ()=>{
  ...
  assert.deepEqual(parseStops("в кино потом кальян").map(s=>s.query),["кинотеатр","кальянная"]);
  assert.deepEqual(parseStops("музей потом кофе").map(s=>s.query),["выставка","кофейня"]);
});

test("вечер через полночь считается верно", async ()=>{
  const search=async()=>({results:[place("b","Бар")]});
  const p=await buildPlan({stops:[{query:"ужин",duration_min:90},{query:"бар",duration_min:120}],start_time:"23:00"},search);
  assert.equal(p.stops[0].slot_start,"23:00");
  assert.equal(p.stops[0].slot_end,"00:30","после полуночи время не уезжает в отрицательное");
  assert.ok(p.stops[1].slot_start>="00:30"||p.stops[1].slot_start<"04:00");
});
